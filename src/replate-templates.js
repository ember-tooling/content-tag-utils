/**
 * @param {string} source
 * @param {ReturnType<typeof import('./parse.js').parse>} templates
 * @param {(innerTemplateContents: string) => string} callback
 */
export function replaceTemplates(source, templates, callback) {
  ...
  for (let parsed of templates) {
    ...
    let originalContent = buffer
      .slice(parsed.range.start, parsed.range.end)
      .toString();
     ...
     * - macros?
     */
    let openingTag = buffer
      .slice(parsed.startRange.start, parsed.startRange.end)
      .toString();
    let closingTag = buffer
      .slice(parsed.endRange.start, parsed.endRange.end)
      .toString();
